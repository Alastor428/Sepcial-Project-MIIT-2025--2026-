...
import { Box, Center, Spinner, Text, HStack, Icon, InputLeftAddon, ChevronLeftIcon } from "native-base";
...
const QRScreen: React.FC = () => {
  ...
  return (
    <Box flex={1} bg="#fff">
      ...
      shadow={2} borderColor={"#7A83F4"} 
      borderWidth={2} 
      marginY={40} marginX={10} mt={10} 
      borderBottomLeftRadius={20} borderBottomRightRadius={20}>
        <Center flex={1}>
          <Text mt={-30} textAlign="center" mb={15} fontSize="lg" color="#B9BDF0">
          {user.name}{" - " +user.userId}
        </Text>
        <Text mt={0} textAlign="center" mb={6} fontSize="lg" color="#7A83F4" fontWeight={"bold"}>
          Scan to Pay Me
        </Text>
        <QRCode
        ...
        <HStack>
        <Pressable onPress={() => navigation.navigate("SetAmountScreen")} mt={10} mr={2}>
          <Text color="#7A83F4" fontSize="md" fontWeight="bold">
            Set Amount
          </Text>
        </Pressable>
        <Pressable onPress={() => navigation.navigate("SaveImage")} mt={10} ml={2}>
          <Text color="#7A83F4" fontSize="md" fontWeight="bold">
            Save Image
          </Text>
        </Pressable>
